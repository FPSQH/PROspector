...
export default function TerrainPage() {
  ...
  const handleResumeSession = async()=>{
    ...
    setLoading(true); setSession(activeSessionFound)
    setIsHorsZone(activeSessionFound.type_session==='hors_zone')
    setActiveDpeFlags([])
    await loadSessionData(activeSessionFound.id)
    setAppState('en_cours'); setLoading(false)
  }

  const handleAbandonAndNew = async()=>{
    if(!activeSessionFound) return
    if(!confirm('Clore la session sans enregistrer ?')) return
    await fetch(`/api/sessions/${activeSessionFound.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({statut:'non_realisee'})})
    try{localStorage.removeItem(SESSION_KEY)}catch(_){}
    setActiveSessionFound(null); setAppState('choix_zone')
  }
  ...
  const handleZonePreview = async(zone:Zone)=>{
    ...
    try{ const res=await fetch(`/api/zones/${zone.id}/adresses`); const d=await res.json(); setPreAdresses(d.adresses??[]) }
  ...
  const handlePreviewHorsZone = async()=>{
    if(!communeChoisie) return
    setPreAdresses([]); setAppState('pre_session_hors_zone'); setPreLoading(true)
    try{ const res=await fetch(`/api/prospection-libre/adresses?code_insee=${communeChoisie.code_insee}`); const d=await res.json(); setPreAdresses(d.adresses??[]) }
    finally{ setPreLoading(false) }
  }
  ...
  const handleStartSession = async(zone?:Zone, horsZone?:boolean)=>{
    ...
    try{
      const body:any = horsZone
        ?{type_session:'hors_zone',commune_code_insee:communeChoisie?.code_insee,commune_nom:communeChoisie?.nom}
        :{zone_id:zone!.id,type_session:'libre'}
      ...
      if(horsZone){
        const adrs=(preAdresses??[]).map((a:any,i:number)=>({...a,statut_carte:'a_faire' as const,ordre:i,prospectable:a.prospectable!==false}))
        setAdresses(adrs); setNbTotal(adrs.length); setNbVisites(0); setPctCouvert(0); setItineraire([])
  ...
  const handleQualification=async(interactionData:any)=>{
    ...
    // Déterminer la couleur du marqueur selon le résultat
    const statut:Adresse['statut_carte']=
      (interactionData.resultat==='contact'||interactionData.resultat==='contact_etabli') ? 'contact' :
    ...
    const wasAlreadyVisited = selectedAdresse.statut_carte !== 'a_faire'
    ...
    if(!isHorsZone) setIdxCourant(prev=>Math.min(prev+1,itineraire.length-1))
  }
  ...
  const allerAdresseSuivante=()=>{
    ...
    const p=adresses.find(a=>a.statut_carte==='a_faire'); if(p){setSelectedAdresse(p);setSheetOpen(true)}
  }
  ...
  const handleEndSession=async()=>{
    if(!session) return
    if(!confirm('Terminer et clôturer cette session ?')) return
    setLoading(true)
    const res=await fetch(`/api/sessions/${session.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({statut:'realisee',nb_portes:nbVisites})})
    const d=await res.json(); setRapport(d.rapport??null)
    try{localStorage.removeItem(SESSION_KEY)}catch(_){}
  ...
  const handlePrepareAddress=()=>{
    ...
    setPendingFormData({...newAddress}); setShowAddressForm(false); setPlacementCoords(null); setPlacingAddress(true)
  }
  ...
  if(appState==='choix_zone') return(
    <div style={{minHeight:'100dvh',background:'#f8f7f4',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'20px 16px'}}>
        ...
        {zones.length===0&&<div style={{textAlign:'center',padding:'32px 0'}}><div style={{fontSize:'2rem',marginBottom:12}}>🗺️</div><p style={{color:'#5F5E5A',fontSize:'0.875rem'}}>Aucune zone configurée</p></div>}
      </div>
    </div>
  )

  // ── Écran choix commune hors zone ─────────────────────────────
  ...
  // ── Écran pré-session zone identifiée ─────────────────────────
    ...
    return(
      <div style={{height:'100dvh',display:'flex',flexDirection:'column',background:'#f8f7f4'}}>
        ...
        <div style={{background:'#fff',borderBottom:'1px solid #e8e7e0',padding:'12px 16px',flexShrink:0}}>
          ...
          {showDpeFilter&&(<>
            <div style={{display:'flex',gap:6,marginBottom:8}}>
              ...
              <button onClick={()=>{setDpeFrom('');setDpeTo('');setPendingFrom('');setPendingTo('')}} style={{padding:'6px 10px',borderRadius:20,fontSize:'0.75rem',border:'1px solid #e8e7e0',background:'transparent',color:'#9b9b96',cursor:'pointer'}}>Effacer</button>
            </div>
            <div style={{display:'flex',gap:8,alignItems:'center'}}>
              ...
              <span style={{color:'#9b9b96',fontSize:'0.8rem'}}>→</span>
        ...
        <div style={{flex:1,position:'relative',overflow:'hidden'}}>
          {preLoading?<div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'#f8f7f4',fontSize:'0.875rem',color:'#9b9b96'}}>Chargement...</div>
          :<TerrainMap adresses={preMap} zonePolygon={null} prochaineAdresseId={null} onAdresseClick={()=>{}} dpeFlags={dpeFlags} dpeFilterFrom={dpeFrom} dpeFilterTo={dpeTo}/>}
        </div>
  ...
  if(appState==='pre_session_hors_zone'&&communeChoisie){
    ...
    return(
      <div style={{height:'100dvh',display:'flex',flexDirection:'column',background:'#f8f7f4'}}>
        ...
        <div style={{flex:1,position:'relative',overflow:'hidden'}}>
          {preLoading?<div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'#f8f7f4',fontSize:'0.875rem',color:'#9b9b96'}}>Chargement des adresses...</div>
          :<TerrainMap adresses={preMap} zonePolygon={null} prochaineAdresseId={null} onAdresseClick={()=>{}} dpeFlags={[]}/>}
    ...
    <div style={{height:'100dvh',display:'flex',flexDirection:'column',background:'#000'}}>
      <div style={{background:'#fff',borderBottom:'1px solid #e8e7e0',padding:'0 12px',height:48,flexShrink:0,display:'flex',alignItems:'center',gap:10,zIndex:10}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:7,flex:1,minWidth:0}}>
          {isHorsZone
            ?<><span style={{fontSize:'0.9rem'}}>🌐</span><span style={{fontWeight:600,fontSize:'0.875rem',color:'#ea580c',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{session?.commune_nom??'Hors Zone'}</span><span style={{fontSize:'0.68rem',fontWeight:600,padding:'1px 5px',borderRadius:8,background:'#fff7ed',color:'#ea580c',flexShrink:0}}>Libre</span></>
            :<><div style={{width:10,height:10,borderRadius:'50%',flexShrink:0,background:session?.zones_prospection?.couleur??'#1D9E75'}}/><span style={{fontWeight:600,fontSize:'0.875rem',color:'#1a1a18',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{session?.zones_prospection?.nom}</span></>
          }
        </div>
        <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
          <div style={{width:80,height:5,background:'#f0efeb',borderRadius:3}}>
            <div style={{width:`${Math.min(pctCouvert,100)}%`,height:'100%',background:isHorsZone?'#ea580c':'#1D9E75',borderRadius:3,transition:'width 0.3s ease'}}/>
          </div>
      ...
      <div style={{flex:1,position:'relative',overflow:'hidden'}}>
        <TerrainMap
          adresses={adresses}
          zonePolygon={null}
          prochaineAdresseId={placingAddress?null:prochaineAdresseId}
          onAdresseClick={placingAddress?()=>{}:handleAdresseClick}
          dpeFlags={activeDpeFlags}
          placementMode={placingAddress}
          onPlacementClick={(lat,lon)=>setPlacementCoords({lat,lon})}
        ...
        {isHorsZone&&showAddressForm&&(
          <div style={{position:'absolute',bottom:0,left:0,right:0,background:'#fff',borderRadius:'20px 20px 0 0',padding:'16px 20px 32px',boxShadow:'0 -4px 20px rgba(0,0,0,0.15)',zIndex:500}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'80px 1fr',gap:8,marginBottom:10}}>
              <input placeholder="N°" value={newAddress.numero} onChange={e=>setNewAddress(a=>({...a,numero:e.target.value}))} style={{padding:'9px 10px',borderRadius:8,border:'1.5px solid #E8E6DF',fontSize:13,outline:'none'}}/>
              <input placeholder="Nom de la voie *" value={newAddress.nom_voie} onChange={e=>setNewAddress(a=>({...a,nom_voie:e.target.value}))} style={{padding:'9px 10px',borderRadius:8,border:'1.5px solid #E8E6DF',fontSize:13,outline:'none'}}/>
            </div>
            <div style={{display:'flex',gap:6,marginBottom:10}}>
              {[['individuel','🏠 Maison'],['collectif','🏢 Immeuble'],['mixte','🏪 Mixte'],['activite','🏭 Activité']].map(([v,l])=>(
                <button key={v} onClick={()=>setNewAddress(a=>({...a,type_habitat:v}))} style={{flex:1,padding:'7px 4px',borderRadius:8,fontSize:11,fontWeight:600,cursor:'pointer',background:newAddress.type_habitat===v?'#1D9E75':'#f3f4f6',color:newAddress.type_habitat===v?'#fff':'#374151',border:'none'}}>{l}</button>
              ))}
            </div>
            {(newAddress.type_habitat==='collectif'||newAddress.type_habitat==='mixte')&&(
              <input type="number" placeholder="Nb boîtes aux lettres" value={newAddress.nb_bal} onChange={e=>setNewAddress(a=>({...a,nb_bal:e.target.value}))} style={{width:'100%',padding:'9px 10px',borderRadius:8,border:'1.5px solid #E8E6DF',fontSize:13,outline:'none',marginBottom:10,boxSizing:'border-box'}}/>
            )}
        ...
        {/* Overlay placement — tap sur la carte */}
          ...
          <div style={{position:'absolute',bottom:0,left:0,right:0,background:'#fff',borderRadius:'20px 20px 0 0',padding:'16px 20px 28px',boxShadow:'0 -4px 20px rgba(0,0,0,0.15)',zIndex:500}}>
            ...
            {!placementCoords?(
              ...
              <div style={{marginBottom:12,padding:'10px 14px',borderRadius:8,background:'#f0fdf4',border:'1px solid #bbf7d0',display:'flex',alignItems:'center',gap:10}}>
                <span style={{fontSize:'1.2rem'}}>✅</span>
                <div style={{flex:1}}><div style={{fontSize:'0.82rem',fontWeight:600,color:'#065f46'}}>Position confirmée · Retapez pour déplacer</div><div style={{fontSize:'0.72rem',color:'#6b7280'}}>{placementCoords.lat.toFixed(5)}, {placementCoords.lon.toFixed(5)}</div></div>
            ...
            <div style={{display:'flex',gap:8}}>
              ...
              <button onClick={handleConfirmPlacement} disabled={!placementCoords||savingAddress} style={{flex:1,padding:'11px',borderRadius:10,fontWeight:700,fontSize:14,background:!placementCoords||savingAddress?'#E8E6DF':'#ea580c',color:'#fff',border:'none',cursor:!placementCoords||savingAddress?'not-allowed':'pointer'}}>
