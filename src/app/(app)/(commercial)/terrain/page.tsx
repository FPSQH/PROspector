...
function calculerItineraire(adresses: Adresse[]): string[] {
  ...
  const runs = Math.min(3, points.length)
...
export default function TerrainPage() {
  ...
  // DPE flags depuis les pré-adresses
  useEffect(() => {
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid #e8e7e0', padding: '12px 16px', flexShrink: 0 }}>
          ...
          {showDpeFilter && (
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
              {[7,14,30,90].map(d => (
                <button key={d} onClick={() => quickSet(d)}
                  style={{ padding: '4px 10px', borderRadius: 20, fontSize: '0.72rem', fontWeight: 600, background: '#f3f4f6', color: '#374151', border: '1px solid #e5e7eb', cursor: 'pointer' }}>
                  {d}j
                </button>
              ))}
              <input type="date" value={pendingFrom} onChange={e => setPendingFrom(e.target.value)}
                style={{ padding: '3px 6px', borderRadius: 6, border: '1px solid #e5e7eb', fontSize: '0.72rem' }} />
  ...
  // ── Écran session en cours ────────────────────────────────────────────────
  const aFaireCnt = adresses.filter(a => a.statut_carte === 'a_faire').length

  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#1a1a18' }}>
      {/* Header session */}
      <div style={{ background: '#fff', padding: '0 16px', height: 52, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid #e8e7e0' }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '0.72rem', color: '#9b9b96' }}>
            {nbVisites}/{nbTotal} · {pctCouvert}% · {aFaireCnt} restantes
          </div>
        </div>
        <button onClick={allerAdresseSuivante}
          style={{ padding: '6px 12px', borderRadius: 8, background: '#f0fdf4', color: '#1D9E75', fontWeight: 600, fontSize: '0.8rem', border: '1px solid #bbf7d0', cursor: 'pointer' }}>
          Suivante
        </button>
        <button onClick={ouvrirGoogleMaps}
          style={{ padding: '6px 12px', borderRadius: 8, background: '#f8f7f4', color: '#374151', fontWeight: 600, fontSize: '0.8rem', border: '1px solid #e8e7e0', cursor: 'pointer' }}>
          🗺
        </button>
        <button onClick={handleEndSession} disabled={loading}
          style={{ padding: '6px 12px', borderRadius: 8, background: loading ? '#e5e7eb' : '#ef4444', color: '#fff', fontWeight: 600, fontSize: '0.8rem', border: 'none', cursor: loading ? 'not-allowed' : 'pointer', flexShrink: 0 }}>
          Terminer
        </button>
      </div>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        {/* Légende — masquée quand la sheet est ouverte */}
        {!sheetOpen && (
          <div style={{
