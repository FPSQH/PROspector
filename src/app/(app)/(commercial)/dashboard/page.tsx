...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('planning_sessions')
      ...
      .order('date_prevue', { ascending: true }).order('heure_debut', { ascending: true })
      .limit(1),

    // Cumul toutes sessions réalisées par zone
    supabase.from('planning_sessions')
          ...
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f0efeb', padding: '18px 24px', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <span style={{ fontSize: '0.72rem', fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: '#d1fae5', color: '#065f46' }}>
                  {mois.nbRealisees} réalisée{mois.nbRealisees > 1 ? 's' : ''}
                </span>
                {mois.nbPlanifiees > 0 && (
                  <span style={{ fontSize: '0.72rem', fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: '#e0f2fe', color: '#0369a1' }}>
                    {mois.nbPlanifiees} planifiée{mois.nbPlanifiees > 1 ? 's' : ''}
                  </span>
                )}
            ...
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f0efeb', padding: '20px 24px' }}>
              ...
              {nbZones === 0 ? (
                ...
                <div style={{ textAlign: 'center', padding: '24px 0', border: '1.5px dashed #e8e7e0', borderRadius: 10 }}>
                  ...
                  <Link href="/planning" style={{ padding: '7px 14px', borderRadius: 8, background: '#1D9E75', color: '#fff', fontWeight: 600, fontSize: '0.8rem', textDecoration: 'none' }}>
                    Générer le planning →
                  </Link>
                </div>
              ) : (
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 16px', background: '#f8fffe', borderRadius: 10, border: '1px solid #e1f5ee' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '0.78rem', color: '#5F5E5A', marginTop: 2 }}>
                      {prochaineZone?.nb_prospectables ?? '—'} adresses · {prochaineSession.heure_debut}–{prochaineSession.heure_fin}
                    </div>
                  </div>
                  <Link href={`/terrain?zone_id=${prochaineSession.zone_id}`} style={{ padding: '8px 14px', borderRadius: 8, background: '#1D9E75', color: '#fff', fontWeight: 600, fontSize: '0.8rem', textDecoration: 'none', flexShrink: 0 }}>
                    Démarrer →
                  </Link>
                </div>
              )}
            ...
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f0efeb', padding: '20px 24px' }}>
              ...
              {nbZones === 0 ? (
                ...
                <>
                    ...
                    {(zones ?? []).slice(0, 10).map((z: any) => {
                      ...
                      return (
                        <div key={z.id} style={{ display: 'grid', gridTemplateColumns: '8px 70px 1fr 48px 52px 52px', gap: '0 10px', alignItems: 'center' }}>
                          ...
                          <div style={{ fontSize: '0.72rem', fontWeight: 600, color: pct >= 80 ? '#065f46' : pct >= 40 ? '#92400e' : '#9b9b96', textAlign: 'right' }}>
                            {visited > 0 ? `${pct}%` : '—'}
                          </div>
                          <div style={{ fontSize: '0.72rem', color: contacts > 0 ? '#FF9800' : '#9b9b96', fontWeight: contacts > 0 ? 600 : 400, textAlign: 'right' }}>
                            {contacts > 0 ? contacts : '—'}
                          </div>
                  ...
                  {nbZones > 10 && (
                    <div style={{ fontSize: '0.75rem', color: '#9b9b96', marginTop: 8 }}>
                      +{nbZones - 10} zones · <Link href="/zones" style={{ color: '#1D9E75' }}>voir tout</Link>
                    </div>
                  )}
                </>
              )}
