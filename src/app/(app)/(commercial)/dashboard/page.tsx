      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 28px' }} className="dash-main">
        ...
        {isManager && (
          <div style={{
            ...
            <div>
              <div style={{ fontWeight: 600, fontSize: '0.9rem', color: '#1a1a18', marginBottom: 3 }}>
                👥 Espace manager
              </div>
              <div style={{ fontSize: '0.8rem', color: '#5F5E5A' }}>
                Gérez les comptes et les accès de votre équipe commerciale.
              </div>
            </div>
        ...
        <div style={{
          ...
          {[
            ...
            { label: 'Prochaine session', value: joursRestants === 0 ? "Aujourd'hui" : `Dans ${joursRestants}j`, sub: `${nomJours[prochainJour]} 10h–12h`, href: null, color: '#FF9800' },
          ].map((kpi: any) => (
            <div key={kpi.label} style={{
              ...
              <div style={{ fontSize: '0.72rem', fontWeight: 500, color: '#9b9b96', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                {kpi.label}
              </div>
              <div style={{ fontSize: '1.5rem', fontWeight: 700, color: kpi.empty ? '#c9c8c2' : '#1a1a18', lineHeight: 1, marginBottom: 4 }}>
                {kpi.value}
              </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 16 }} className="dash-grid">
            ...
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f0efeb', padding: '20px 24px' }}>
              ...
              {nbZones === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px 0', border: '1.5px dashed #e8e7e0', borderRadius: 10 }}>
                  ...
                  <div style={{ fontSize: '0.875rem', color: '#5F5E5A', marginBottom: 4 }}>Aucune zone configurée</div>
                  <div style={{ fontSize: '0.78rem', color: '#9b9b96', marginBottom: 16 }}>Générez vos zones pour planifier vos tournées</div>
                  <Link href="/zones" style={{ padding: '8px 16px', borderRadius: 8, background: '#1D9E75', color: '#fff', fontWeight: 600, fontSize: '0.82rem', textDecoration: 'none' }}>
                    Générer les zones
                  </Link>
                </div>
              ) : zoneRecommandee ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 16px', background: '#f8fffe', borderRadius: 10, border: '1px solid #e1f5ee' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 10, background: zoneRecommandee.couleur + '20', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <div style={{ width: 14, height: 14, borderRadius: '50%', background: zoneRecommandee.couleur }}/>
                  </div>
                  <div style={{ flex: 1 }}>
              ...
              {nbZones === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {(zones ?? []).slice(0, 8).map((z: any) => {
                    ...
                    const barColor = ratio > 2 ? '#ef4444' : ratio > 1.3 ? '#f59e0b' : '#1D9E75'
                    ...
                    return (
                      <div key={z.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div style={{ fontSize: '0.78rem', color: '#1a1a18', width: 70, flexShrink: 0 }}>{z.nom}</div>
                        <div style={{ flex: 1, height: 5, background: '#f0efeb', borderRadius: 3, overflow: 'hidden' }}>
                          <div style={{ width: `${pct}%`, height: '100%', background: barColor, borderRadius: 3 }}/>
                        </div>
                        <div style={{ fontSize: '0.72rem', color: '#9b9b96', width: 36, textAlign: 'right', flexShrink: 0 }}>{val}</div>
                      </div>
                    )
                  })}
                  {nbZones > 8 && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f0efeb', padding: '20px 24px' }}>
              ...
              <Link href="/onboarding" style={{ display: 'block', marginTop: 12, fontSize: '0.75rem', color: '#1D9E75', textDecoration: 'none' }}>
                Gérer le secteur →
              </Link>
            </div>
            ...
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f0efeb', padding: '20px 24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {([
                  ...
                  { href: '/zones/edit', label: 'Editer les zones',  icon: '✏️', active: nbZones > 0 },
                  { href: '/onboarding', label: 'Gerer le secteur',  icon: '🏘️', active: true },
                  ...(isManager ? [{ href: '/admin/users', label: 'Gerer l equipe', icon: '👥', active: true }] : []),
                  ...
                  action.active ? (
                    ...
                    <div key={action.href} style={{
                      display: 'flex', alignItems: 'center', gap: 10,
                      padding: '9px 12px', borderRadius: 8,
                      background: '#fafaf8', fontSize: '0.82rem', color: '#c9c8c2', cursor: 'not-allowed',
                    }}>
