...
export async function POST() {
  ...
  // Grouper par adresse_id : type_bien majoritaire + has_commerce si au moins un immeuble
  ...
  if (!adresseMap.size) return NextResponse.json({ nb_qualifiees: 0, nb_dpe_traites: dpeRows.length, nb_adresses_distinctes: 0 })
  ...
  // Appliquer les updates — ne pas ecraser les qualifications manuelles
  // Une qualification manuelle = type_bien different de 'inconnu' et non null
  // On qualifie : type_bien IS NULL ou type_bien = 'inconnu'
  let nbQualifiees = 0
  ...
  for (const [key, ids] of groups) {
    ...
    for (let i = 0; i < ids.length; i += 100) {
      ...
      // Qualifier les adresses sans qualification (null ou 'inconnu')
