...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase.from('projets_immobiliers').update(update).eq('id', params.id).eq('commercial_id', effectiveId).select().single()
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  const effectiveId = await getEffectiveCommercialId()
  const { error } = await supabase.from('projets_immobiliers').delete().eq('id', params.id).eq('commercial_id', effectiveId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
