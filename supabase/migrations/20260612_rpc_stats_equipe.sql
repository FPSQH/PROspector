-- ============================================================
-- Migration : RPC get_stats_equipe()
-- Date      : 2026-06-12
-- Objectif  : Stats agrégées par commercial pour le dashboard
--             manager — une seule requête, pas de N+1
-- ============================================================

CREATE OR REPLACE FUNCTION get_stats_equipe(
  p_manager_id UUID,
  p_debut      DATE DEFAULT NULL,  -- NULL = tout le temps
...
AS $$
  SELECT
    ...
    -- Mandats signés
    COUNT(i.id) FILTER (
      WHERE i.statut_adresse = 'mandat_signe'
        AND (p_debut IS NULL OR s.date_session >= p_debut)
        AND (p_fin   IS NULL OR s.date_session <= p_fin)
    )                                 AS nb_mandats,

    -- Contacts chauds (horizon ≤ 6 mois, sans filtre période)
    (
      SELECT COUNT(*)
      FROM contacts ct
      WHERE ct.commercial_id = c.id
        AND ct.horizon_vente IN ('immediat','3_mois','6_mois')
    ...
    COALESCE((
      SELECT ROUND(AVG(
        CASE WHEN z.nb_prospectables > 0
          THEN z.nb_adresses::NUMERIC / z.nb_prospectables * 100
          ELSE 0
        END
      ), 1)
  ...
  FROM commerciaux c
  -- Vérifie que le manager appelant est bien le manager de ce commercial
